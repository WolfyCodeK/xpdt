...
for arg in "$@"; do
  case "$arg" in
    -h | --help)
      cat <<'USAGE'
xpdt - a customised xplr (terminal file manager) set up as a keyboard-driven
git client and code browser. It runs:  xplr -c ~/.config/xpdt/init.lua

Usage:
  xpdt [PATH] [SELECTION]...   open xpdt, optionally focused on PATH
  xpdt --help                  show this message
  xpdt --version               show the underlying xplr version
