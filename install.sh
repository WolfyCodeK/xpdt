...
# xplr at the custom config via -c. Generated with the resolved xplr path baked in.
install_launcher() {
  cat > "$TMP_DIR/xpdt" <<EOF
#!/bin/sh
# xpdt - xplr running the custom xpdt config. Generated by xpdt install.sh; edits are overwritten.
xplr_bin="$BIN_DIR/xplr"
