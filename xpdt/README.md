...
Commit history (`;`): `→` open a commit, `b` view a different branch's / ref's commits (local or remote), `ctrl-p` cherry-pick the focused commit onto the current branch, `ctrl-z` undo the last commit (soft reset), `←` back. Inside a commit: `→` inline diff viewer, `enter` open the diff in VS Code, `←` back. The header shows which branch you are viewing and which branch a cherry-pick would land on.
...
| `git-log-list.sh`        | Prints the commit list for the `;` browser; optional 2nd arg is a branch/ref to view instead of the current HEAD.                                                                                                                                                                                                                           |
...
| `claude-status.sh`       | For the `claude-integration` setting: prints a short `◆ claude · <branch>` line if a Claude Code session has been active in the given repo recently. Scans `~/.claude/projects/*/*.jsonl` (each records the session `cwd` and `gitBranch`) for files modified in the last 10 min whose cwd is under the repo. Off unless the setting is on. |
...
Another opt-in setting is **claude-integration** (default off, so `gate.sh get` special-cases it): when on, the git-history panel shows a `◆ claude · <branch>` line if a Claude Code session has recently been active in the current repo. Claude Code stores each session as `~/.claude/projects/<encoded-cwd>/<id>.jsonl` recording the session `cwd` and `gitBranch`; `claude-status.sh` finds the ones modified in the last 10 minutes whose cwd is under the repo and prints the marker plus their branch(es). `init.lua` caches the result per repo (`CLAUDE_TTL`) and prepends it to the panel body, and skips the scan entirely when the setting is off (`read_bool_setting` guard, no spawn). It uses only the session files - no tmux, no process inspection - so it works on a plain single-machine install.
