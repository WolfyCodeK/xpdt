...
Every mutating action (create / move / delete, stage / discard / commit, stash apply / pop / drop / new / clear, undo commit, git checkout / pull) runs behind the confirmation gate: by default it asks you to type two random digits first. The `,` menu turns that off globally or per action. See Confirmation gate below.

Changes browser (`enter`): `s` stage/unstage (toggle, based on where the entry currently is), `d` discard, `c` commit (prompts for a message), `ctrl-e` edit the file in Neovim, `→` inline diff viewer, `enter` open the diff in VS Code, `←` back. Stage / discard / commit go through the confirmation gate.
...
- The actions covered: `create`, `move`, `delete` (main view); `stage`, `discard`, `commit` (changes browser); `undo` (commit history); `stash-apply`, `stash-pop`, `stash-drop`, `stash-new`, `stash-clear` (stash browser); `checkout`, `pull` (git menu).
