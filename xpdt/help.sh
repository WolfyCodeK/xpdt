...
sec() { printf "\n  ${H}%s${Z}\n" "$1"; }
{
  ...
  row "d" "delete (to the Trash where the platform has one)"
  ...
  row "right" "open in neovim at the line"
  row "ctrl-e" "same as right"
  row "ctrl-v" "mark the start of a line range (then move and press ctrl-y)"
  row "ctrl-y" "copy the marked range, or the whole file, to the clipboard"
  row "left" "back"
  ...
  row "left" "close"
  sec "NOT BOUND"
  row "" "xplr's own r (rename), ctrl-d (duplicate) and ctrl-w (switch layout)"
  row "" "are turned off - r and ctrl-d wrote to disk with no confirm code,"
  row "" "and ctrl-w replaced this whole layout. Use m to rename, a to create."
  printf "\n  ${D}%s${Z}\n" "(press q, esc or left to close)"
